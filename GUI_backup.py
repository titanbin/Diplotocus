...
from PySide6.QtWidgets import (
    ...
    QLabel, QSizePolicy,QSplitter, QPushButton, QDialog, QLineEdit, QFormLayout, QComboBox
)
from PySide6.QtCore import Qt, QPointF, QRectF, Signal, QObject, QMimeData, QEvent, QTimer, Slot, QThread, QSize
from PySide6.QtGui import QBrush, QColor, QPen, QPixmap, QDrag, QIcon, QKeySequence, QShortcut, QGuiApplication, QFontMetrics
...
class TimelineClip(QGraphicsRectItem):
    def __init__(self, timeline, x, row, name):
        self.timeline = timeline
        self.plot_object = None
        super().__init__(0, 0, CLIP_WIDTH, self.timeline.row_height)
        self.just_spawned = True

        self.base_color = self.get_color_for_name(name)
        self.name = name
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        bottom_layout.setContentsMargins(0, 0, 50, 0)
        
        self.list_widget = PaletteList()
        self.list_widget.addItems(["Translate", "Rotate", "Scale"])
    ...
    def move_to_screen(self, screen_index=1):
        ...
        if len(screens) > screen_index:
            ...
            if hasattr(self, 'viewport') and callable(getattr(self, 'viewport')):
